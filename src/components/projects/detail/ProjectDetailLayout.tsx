...
function ImageGalleryPlaceholder({ alt }: { alt: string }) {
  return (
    <div className="relative h-52 w-full rounded-xl overflow-hidden bg-neutral-900 border border-white/[0.07]">
      <div
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
        ...
        <span className="text-[10px] text-neutral-700">{alt}</span>
      </div>
    </div>
  );
}
    ...
    <article className="py-16 max-w-2xl mx-auto space-y-12">
      ...
      >
        <div className="space-y-1">
          {project.period && (
            <p className="text-[10px] text-neutral-600 font-mono tracking-wider">{project.period}</p>
          )}
          <h1 className="text-2xl sm:text-3xl font-bold text-white leading-tight">
            {project.title}
          </h1>
          {project.role && (
            <p className="text-sm text-neutral-400">{project.role}</p>
          )}
        </div>
        ...
        {project.relatedRoles && project.relatedRoles.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {project.relatedRoles.map((r) => (
              <span
                key={r}
                className="px-2.5 py-0.5 rounded-full text-[10px] text-blue-400 bg-blue-500/10 border border-blue-500/20"
              >
                {r}
              </span>
            ))}
      ...
      >
          ...
          {project.imageGallery && project.imageGallery.length > 0 ? (
            <div className="grid gap-3 sm:grid-cols-2">
              {project.imageGallery.map((src, i) => (
                <div key={i} className="relative h-48 w-full rounded-xl overflow-hidden border border-white/[0.07]">
                  <Image
                    src={src}
        ...
        {project.learned && (
          <Section title="What I Learned" label="배운 점">
            <p className="text-sm text-neutral-400 leading-relaxed">{project.learned}</p>
          </Section>
        )}
          ...
          <div className="flex gap-3 pt-2">
            {project.githubHref && (
              <a
                ...
                className="px-4 py-2 rounded-xl text-sm text-neutral-300 bg-white/[0.05] border border-white/[0.09] hover:bg-white/[0.08] transition-colors"
              >
                GitHub →
              </a>
            )}
            {project.href && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="px-4 py-2 rounded-xl text-sm font-medium text-black bg-white hover:bg-neutral-200 transition-colors"
              >
                Live →
              </a>
            )}
  ...
  return (
    <div className="space-y-3">
      <div className="space-y-0.5">
        <p className="text-[10px] text-neutral-600 tracking-widest uppercase">{title}</p>
