    ...
    <section className="py-16">
      ...
      >
        <div className="space-y-1">
          <p className="text-[10px] text-neutral-600 tracking-widest uppercase">Background</p>
          ...
          <div className="space-y-6 pl-7">
            {experienceItems.map((item, i) => (
              ...
              >
                ...
                {/* Card */}
                <div className="rounded-2xl p-5 bg-white/[0.04] border border-white/[0.08] space-y-3 hover:border-white/[0.12] transition-colors">
                  <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-1">
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-sm font-medium text-white">{item.role}</span>
                        <span className="text-[10px] text-neutral-600 px-2 py-0.5 rounded-full bg-white/[0.04] border border-white/[0.07]">
                          {typeLabel[item.type]}
                        </span>
                      </div>
                      <span className="text-xs text-neutral-400">{item.org}</span>
                    </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {item.keywords.map((kw) => (
                      <span
                        key={kw}
                        className="px-2 py-0.5 rounded-full text-[10px] text-neutral-400 bg-white/[0.04] border border-white/[0.07]"
                      >
                        {kw}
                      </span>
                    ))}
