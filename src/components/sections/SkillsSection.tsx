    ...
    >
      ...
      <div className="flex flex-col gap-6 flex-1 justify-center">
        {techStack.map((category, ci) => (
          ...
          >
            <p className="text-xs tracking-[3px] uppercase mb-2" style={{ color: "var(--muted)" }}>
              {category.label}
            </p>
            <div className="flex flex-wrap gap-[6px]">
              {category.items.map((item) => (
                <span
                  key={item.name}
                  className="text-xs px-3 py-[5px] rounded-[2px] border font-medium transition-colors duration-150"
                  style={{
